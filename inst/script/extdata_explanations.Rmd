...
These files are all from a toy example created by a member of the Johnson lab. The `virus_example.fastq` file contains 150 reads from six different samples, all labeled according to different viruses. The files `barcodes.txt` and `virus_example_index.fastq` are used in the demultiplexing process. Detailed information on how to reproduce this data is unfortunately unavailable, but it is a relatively small dataset and only intended for use as a toy example.

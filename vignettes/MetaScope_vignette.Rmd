...
MetaScope will fun faster and more efficiently if samtools is present on your device!
You can download it at [http://www.htslib.org/](http://www.htslib.org/).
...
sapply(all_species, MetaScope::download_refseq, 
       reference = FALSE, representative = FALSE, compress = FALSE)
```
...
MetaScope::metascope_id(final_map, input_type = "bam", 
                        # change input_type to "csv.gz" when not creating a BAM
                        aligner = "bowtie2", num_species_plot = 0)
```
...
relevant_col <- read.csv(paste0(tools::file_path_sans_ext(final_map),
                                ".metascope_id.csv"))[,2:4]

relevant_col |>
  dplyr::mutate(Genome = stringr::str_replace_all(Genome, ',.*', ''),

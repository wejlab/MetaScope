...
After acquiring the target and filtering genome libraries, we will be able to take the sequencing reads from our sample and map them first to a target library, and then to the filter library. All reads that map to the filter library with a score equal or greater than the mapping score for the target library will be discarded; all reads mapping with a better score to the target library will be used downstream for further sample analysis.
